            ...
            if (outPortDataLen > 0)
            {
               ...
               if (outDataFile != 0)
               {
                  ...
                  else
                  {
                     ...
                     {
                        APX_LOG_ERROR("[APX_NODE_MANAGER] %s connected since before. Keeping previous provide values until out file is opened", apxNode->name);
                     }
                     apx_nodeData_setNodeInfo(outDataFile->nodeData, nodeInfo);
